...
All default warnings are enabled in a block by either of these two
equivalent statements:

    use warnings;
    use warnings 'all';

Similarly all default warnings are disabled in a block by either of
these two equivalent statements:

    no warnings;
    no warnings 'all';

The "default warnings" are almost all of them. Between perl releases
we may test out new optional warnings in the "new" category. Those
have to be enabled explicitly. See L</New Warnings> for details.

As an example of enabling and disabling warnings, consider the code
below:
...
Note: Before 5.20.0, the "missing" lexical warnings category was
internally defined to be the same as the "uninitialized" category. It
is now a top-level category in its own right.

=head2 New Warnings

When new experimental warnings are added to Perl they may be added to
the "new" category instead of the "all" category, so to really enable
all warnings you need to do:

    use warnings qw(all new);

This allows for merging patches for new warnings that may have been
added relatively late in a release cycle without having given CPAN
authors sufficient time to deal with them.

These "new" warnings are very likely to just be migrated to the "all"
category on the next major perl release.
