...
sub run_tests {
  ...
  SKIP:
...

    {
        ...
        my $strasse = "stra" . latin1_to_native("\337") . "e";
  ...
  SKIP:
...

    {
        ...
        ok "<\x{100}" . latin1_to_native("\x{0085}") . ">" =~ /<\x{100}\s>/, '\x{0085} in \s';
        ok        "<" . latin1_to_native("\x{0085}") . ">" =~        /<\v>/, '\x{0085} in \v';
        ok "<\x{100}" . latin1_to_native("\x{00A0}") . ">" =~ /<\x{100}\s>/, '\x{00A0} in \s';
        ok        "<" . latin1_to_native("\x{00A0}") . ">" =~        /<\h>/, '\x{00A0} in \h';
    ...
    BEGIN {
        ...
        $name = "A" . latin1_to_native("\x{D7}") . "O";
        eval "q(W) =~ /\\N{$name}/";
        ...
        $utf8_name = "A HOUSE " . latin1_to_native("\xF7") . " AGAINST ITSELF";
        utf8::upgrade($utf8_name);
        eval "use utf8; q(W) =~ /\\N{$utf8_name}/";
        ...
        undef $w;
        $name = "A" . latin1_to_native("\x{D1}") . "O";
        eval "q(W) =~ /\\N{$name}/";
    ...
    {
        my @ary = (
            pack('U', ord_latin1_to_native(0x00F1)), # n-tilde
            '_'.pack('U', ord_latin1_to_native(0x00F1)), # _ + n-tilde
            'c'.pack('U', 0x0327),        # c + cedilla
            pack('U*', ord_latin1_to_native(0x00F1), 0x0327),# n-tilde + cedilla
            pack('U', 0x0391),            # ALPHA
...

    {
        ...
        for my $c ("z", "\0", "!", chr(254), chr(256)) {
            my $targ = "a" . latin1_to_native("\034") . "$c";
            my $reg  = "a\\c\\$c";
    ...
    {   # More checking that /i works on the few properties that it makes a
        ...
        ok latin1_to_native("\xC0") =~ /\p{XPosixUpper}/,  "\\xC0 =~ XPosixUpper";
        ok latin1_to_native("\xC0") =~ /\p{XPosixUpper}/i,  "\\xC0 =~ XPosixUpper under /i";
        ok latin1_to_native("\xC0") !~ /\p{XPosixLower}/,  "\\xC0 !~ XPosixLower";
        ok latin1_to_native("\xC0") =~ /\p{XPosixLower}/i,  "\\xC0 =~ XPosixLower under /i";
        ok latin1_to_native("\xE0") !~ /\p{XPosixUpper}/,  "\\xE0 !~ XPosixUpper";
        ok latin1_to_native("\xE0") =~ /\p{XPosixUpper}/i,  "\\xE0 =~ XPosixUpper under /i";
        ok latin1_to_native("\xE0") =~ /\p{XPosixLower}/,  "\\xE0 =~ XPosixLower";
        ok latin1_to_native("\xE0") =~ /\p{XPosixLower}/i,  "\\xE0 =~ XPosixLower under /i";

        ok latin1_to_native("\xC0") =~ /\p{UppercaseLetter}/,  "\\xC0 =~ UppercaseLetter";
        ok latin1_to_native("\xC0") =~ /\p{UppercaseLetter}/i,  "\\xC0 =~ UppercaseLetter under /i";
        ok latin1_to_native("\xC0") !~ /\p{LowercaseLetter}/,  "\\xC0 !~ LowercaseLetter";
        ok latin1_to_native("\xC0") =~ /\p{LowercaseLetter}/i,  "\\xC0 =~ LowercaseLetter under /i";
        ok latin1_to_native("\xC0") !~ /\p{TitlecaseLetter}/,  "\\xC0 !~ TitlecaseLetter";
        ok latin1_to_native("\xC0") =~ /\p{TitlecaseLetter}/i,  "\\xC0 =~ TitlecaseLetter under /i";
        ok latin1_to_native("\xE0") !~ /\p{UppercaseLetter}/,  "\\xE0 !~ UppercaseLetter";
        ok latin1_to_native("\xE0") =~ /\p{UppercaseLetter}/i,  "\\xE0 =~ UppercaseLetter under /i";
        ok latin1_to_native("\xE0") =~ /\p{LowercaseLetter}/,  "\\xE0 =~ LowercaseLetter";
        ok latin1_to_native("\xE0") =~ /\p{LowercaseLetter}/i,  "\\xE0 =~ LowercaseLetter under /i";
        ok latin1_to_native("\xE0") !~ /\p{TitlecaseLetter}/,  "\\xE0 !~ TitlecaseLetter";
        ok latin1_to_native("\xE0") =~ /\p{TitlecaseLetter}/i,  "\\xE0 =~ TitlecaseLetter under /i";
...

    {
        ...
        my @df = (chr ord_latin1_to_native(0xDF), '-', chr ord_latin1_to_native(0xDF));
        utf8::upgrade ($df [2]);
        my @strs = ('ss', 'sS', 'Ss', 'SS', chr ord_latin1_to_native(0xDF));
        my @ss = map {("$_", "$_")} @strs;
...

    {
        ...
        my $_ = "aoeu " . latin1_to_native("\xe6") . "var ook";
        /^ \w+ \s (?<eek>\S+)/x;
...
# more TRIE/AHOCORASICK problems with mixed utf8 / latin-1 and case folding
    for my $ord (160 .. 255) {
        my $chr = ord_latin1_to_native($ord);
        my $chr_byte = chr($chr);
...
#            ]
    ...
    {   # This was failing unless an explicit /d was added
        my $E0 = latin1_to_native("\xE0");
        my $p = qr/[_$E0]/i;
        utf8::upgrade($p);
        like(latin1_to_native("\xC0"), $p, "Verify \"\\xC0\" =~ /[\\xE0_]/i; pattern in utf8");
    }
